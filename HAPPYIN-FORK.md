...
| `label_studio/tasks/management/commands/cars_backfill_pred_coverage.py` | One-time/idempotent backfill: writes `Task.data['pred_coverage']` AND `Task.data['pred_score']` (max prediction confidence, 0..1) per project **and registers both DM columns** on the project summary (so they show + filter/sort). `--dry-run` prints the distribution; `--only-missing` for post-import top-ups |
| `.scratch/backfill_pred_boxes.py` (one-off) | Writes `Task.data['pred_boxes'] = {w,h,b:[[x,y,bw,bh,label]]}` (bbox % + natural dims) so the card overlay can draw the prediction without loading full predictions |
| `label_studio/data_manager/api.py` · `CarsBulkAcceptAPI` | `POST /api/dm/tasks/bulk-accept/` — two modes: `{project, task_ids:[...]}` accepts those specific (the grid's VISIBLE/filtered) tasks; or `{project, min_coverage}` accepts ALL project tasks ≥ coverage. Creates one annotation from each task's latest prediction; skips already-annotated; `dry_run` for counts |
...
| `web/libs/editor/src/tags/control/Choice.jsx` | Fast-verify: picking a verdict (by KEY **or** MOUSE) auto-submits the annotation (instead of select + Ctrl+Enter). Logic lives in `toggleSelected()` so it covers hotkey, click and dropdown; scoped to a `Choices` control **named `verdict`**, and only fires on user toggle (draft/annotation restore uses `setSelected()`), so normal/multi-step labeling is unaffected. Mirrors the BottomBar submit/update decision. Stops verification verdicts piling up as un-submitted drafts |
